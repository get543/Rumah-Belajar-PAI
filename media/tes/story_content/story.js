...
var IE =
  (document.all && strAppVersion.indexOf("MSIE") != -1) || strAgent.indexOf("Trident") != -1
    ? true
    : false;
var IE6 = document.all && strAppVersion.indexOf("MSIE 6.") != -1 ? true : false;
var gtIEWin7 =
  IE &&
  strAgent.indexOf("Windows NT 6.1") == -1 &&
  strAgent.indexOf("Windows NT 6.0") == -1 &&
  strAgent.indexOf("Windows NT 5.1") == -1 &&
  strAgent.indexOf("Windows NT 5.0") == -1;
...
if (g_bLMS) {
  document.write(
    "<SCR" + "IPT LANGUAGE='JavaScript1.2' SRC='lms/lms.js' TYPE='text/javascript'></SCR" + "IPT>"
  );
}
...
if (g_bAOSupport) {
  document.write(
    "<SCR" +
      ...
      g_strContentFolder +
      "/AOComm.js' TYPE='text/javascript'></SCR" +
      "IPT>"
  );
}
...
function WriteSwfObject(
  ...
  strHtml +=
    "<div" + strRole + " style='width:" + strWidth + "; height:" + strHeight + ";' id='divSwf' >";
  strHtml +=
...
function RestoreWebObjects() {
  ...
  for (var i = 0; i < g_arrStoredWebObjects.length; i++) {
    ...
    OpenWebObject(
      oWebObject.strId,
      oWebObject.strInitUrl,
      oWebObject.Position.XPos,
      oWebObject.Position.YPos,
      oWebObject.Position.Width,
      oWebObject.Position.Height,
      oWebObject.Position.SlideXOffset,
      oWebObject.Position.SlideYOffset
    );
  }
...
function OpenSwfWindow(
  ...
  g_wndSwf = window.open(
    GetBasePath() + g_strContentFolder + "/swf.html?" + strSearch,
    "SwfWnd",
    strOptions
  );
  g_wndLast = g_wndSwf;
...
function OpenVideo(
  ...
  g_wndSwf = window.open(
    GetBasePath() + g_strContentFolder + "/VideoPlayer.html?" + strSearch,
    "Video",
    strOptions
  );
  g_wndLast = g_wndSwf;
...
function EmailResults(
  ...
  var strMainHeader =
    ...
    strTitle +
    strNewLine +
    "Status, Score, Passing Score, Max Score, Min Score, Time" +
    strNewLine;
  var strLineHeader =
    strNewLine +
    strNewLine +
    "Date, Time, Score, Quiz Name, Interaction ID, Interaction Type, Student Response, Result, Weight, Latency" +
    strNewLine;
  ...
  if (IE) {
    sHTML +=
      '<FORM id="formQuiz" method="POST" action="mailto:' +
      strAddress +
      "?subject=" +
      g_strSubject +
      "&amp;body=" +
...
function SendRequest(oTinCanRequest) {
  if (g_bWaitingTinCanResponse || g_bStopPosting) {
    ...
    PostAsyncRequest(
      ...
      oTinCanRequest.Method,
      oTinCanRequest.Data,
      oTinCanRequest.Url,
      oTinCanRequest.Headers
    );
  }
}
...
function PostAsyncRequest(nMessageType, strMethod, strData, strUrl, arrHeaders) {
  try {
    ...
    if (
      confirm(
        "Unable to connect to server.  Please verify that you are connected to the internet. Retry?"
      )
    ) {
...
function UseXDomainRequest(strUrl) {
  ...
  if (window.XDomainRequest) {
    ...
    bResult =
      anchorDest.hostname != anchorSource.hostname ||
      anchorDest.protocol != anchorSource.protocol ||
      anchorDest.port != anchorSource.port;
  }
